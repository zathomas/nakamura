 ...
 *
 */
public class RestLoginModulePluginTest {
  RestLoginModulePlugin plugin;

  SimpleCredentials credentials;

  @Before
  public void setUp() throws Exception {
    plugin = new RestLoginModulePlugin();

    credentials = new SimpleCredentials("someUser", new char[0]);
    credentials.setAttribute(RestPrincipal.class.getName(), new RestPrincipal("someUser"));
  }
  ...
  public void cannotGetAuthenticationWrongPrincipalType() throws Exception {
    SimpleCredentials credentials = new SimpleCredentials("someUser", new char[0]);
    credentials.setAttribute(RestPrincipal.class.getName(), mock(Principal.class));
    plugin.getAuthentication(null, credentials);
  }
}
