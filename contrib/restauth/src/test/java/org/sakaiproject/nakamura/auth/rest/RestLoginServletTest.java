...
public class RestLoginServletTest {
  RestLoginServlet servlet;
  TrustedTokenServiceImpl trustedTokenService;

  @Mock
  RestAuthenticationHandler handler;
  ...
  public void setUp() throws Exception {
    ...
    servlet = new RestLoginServlet(handler, trustedTokenService);
  ...
  public void redirectWithoutTarget() throws Exception {
    when(request.getAuthType()).thenReturn(RestAuthenticationHandler.AUTH_TYPE);

    ArgumentCaptor<String> redirectCaptor = ArgumentCaptor.forClass(String.class);

    when(request.getContextPath()).thenReturn("someContextPath");
    when(request.getAttribute(RestAuthenticationHandler.AUTHN_INFO)).thenReturn(null);
    servlet.service(request, response);
  ...
  public void authAndAddToken() throws Exception {
    ...
    when(request.getAttribute(RestAuthenticationHandler.AUTHN_INFO)).thenReturn(authnInfo);
    when(request.getRequestURI()).thenReturn("someURI");
    when(request.getAttribute(Authenticator.LOGIN_RESOURCE)).thenReturn("greatplace");
    servlet.service(request, response);
