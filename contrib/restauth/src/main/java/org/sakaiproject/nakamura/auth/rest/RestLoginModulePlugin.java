...
import org.sakaiproject.nakamura.auth.rest.RestAuthenticationHandler.RestPrincipal;
...
@Component
@Service
public class RestLoginModulePlugin implements LoginModulePlugin {
  ...
  public AuthenticationPlugin getAuthentication(Principal principal,
    ...
    if (canHandle(credentials)) {
      plugin = new RestAuthenticationPlugin(this);
    }
    return plugin;
  }

  public Principal getPrincipal(Credentials credentials) {
    RestPrincipal ssoPrincipal = null;
    if (credentials instanceof SimpleCredentials) {
      SimpleCredentials simpleCredentials = (SimpleCredentials) credentials;
      Object attribute = simpleCredentials.getAttribute(RestPrincipal.class.getName());
      if (attribute instanceof RestPrincipal) {
        ssoPrincipal = (RestPrincipal) attribute;
      }
    }
    return ssoPrincipal;
  }
