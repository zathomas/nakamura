...
public class RestLoginServlet extends SlingAllMethodsServlet {
  private static final long serialVersionUID = -1894135945816269913L;
  private static final Logger LOGGER = LoggerFactory.getLogger(RestLoginServlet.class);

  public static final String TRY_LOGIN = "sakaiauth:login";

  @Reference
  private RestAuthenticationHandler ssoAuthnHandler;
  ...
  RestLoginServlet(RestAuthenticationHandler ssoAuthHandler,
  ...
  protected void service(SlingHttpServletRequest request,
    ...
    if (request.getAuthType() != null) {
      AuthenticationInfo authnInfo = (AuthenticationInfo) request
          .getAttribute(RestAuthenticationHandler.AUTHN_INFO);
      if (authnInfo != null) {
        RestAuthenticationTokenServiceWrapper tokenServiceWrapper = new RestAuthenticationTokenServiceWrapper(
